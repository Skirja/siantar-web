...
interface DataContextType {
  ...
  updateOutlet: (
    id: string,
    outlet: Partial<TablesInsert<"outlets">>,
  ) => Promise<void>;
  ...
  updateOrderPayment: (
    orderId: string,
    paymentData: { payment_proof_url?: string; payment_status?: string },
  ) => Promise<void>;
  updateOrder: (
    orderId: string,
    updates: Partial<TablesInsert<"orders">>,
  ) => Promise<void>;
  assignDriver: (
    orderId: string,
    driverId: string,
  ...
  submitOrderRating: (
    rating: Omit<OrderRating, "id" | "created_at">,
  ) => Promise<void>;
...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const refreshProducts = useCallback(async () => {
    ...
    const productsWithDetails: ProductWithDetails[] = (prods || []).map(
      (p) => ({
        ...p,
  ...
  const refreshOrderRatings = useCallback(async () => {
    const { data } = await (supabase.from as any)("order_ratings")
      .select("*")
      .order("created_at", { ascending: false });
  ...
  useEffect(() => {
    ...
    const productsChannel = subscribeToTable("products", () =>
      refreshProducts(),
    );
    ...
    const outletsChannel = subscribeToTable("outlets", (payload) => {
      if (payload.eventType === "UPDATE") {
        setOutlets((prev) =>
          prev.map((o) =>
            o.id === payload.new.id ? ({ ...o, ...payload.new } as Outlet) : o,
          ),
        );
      } else if (payload.eventType === "INSERT") {
    ...
    const appSettingsChannel = subscribeToTable("app_settings", () =>
      refreshAppSettings(),
    );
    const feeSettingsChannel = subscribeToTable("fee_settings", () =>
      refreshFeeSettings(),
    );
    ...
    */
    const pollingInterval = undefined as unknown as ReturnType<
      typeof setInterval
    >;
  ...
  const addOutlet = useCallback(
    async (outlet: TablesInsert<"outlets">) => {
  ...
  const updateOutlet = useCallback(
    async (id: string, outlet: Partial<TablesInsert<"outlets">>) => {
  ...
  const toggleOutletOpen = useCallback(
    async (id: string) => {
  ...
  const getProductsByOutlet = useCallback(
    (outletId: string) => {
  ...
  const addOrder = useCallback(
    async (
      ...
      const { data: orderId, error } = await supabase.rpc("create_order", {
        ...
        p_customer_note: (order as any).customer_note || null,
      });
  ...
  const updateOrderStatus = useCallback(
    async (orderId: string, status: OrderStatus, driverId?: string) => {
  ...
  const updateOrder = useCallback(
    async (orderId: string, updates: Partial<TablesInsert<"orders">>) => {
      const { error } = await supabase
        .from("orders")
        .update(updates)
        .eq("id", orderId);
  ...
  const updateDriver = useCallback(
    async (id: string, driver: Partial<Profile>) => {
      const { error } = await supabase
        .from("profiles")
        .update(driver)
        .eq("id", id);
  ...
  const updateDriverBalance = useCallback(
    async (driverId: string, amount: number) => {
  ...
  const driverRejectOrder = useCallback(
  ...
  const toggleDriverOnline = useCallback(
    async (driverIdParam: string): Promise<boolean> => {
  ...
  const updateDriverLocation = useCallback(
    async (driverIdParam: string, latitude: number, longitude: number) => {
      const { error } = await supabase
        .from("profiles")
        .update({
          latitude,
  ...
  const completeOrderWithDeduction = useCallback(
    async (orderId: string, driverIdParam: string) => {
  ...
  const submitOrderRating = useCallback(
    async (rating: Omit<OrderRating, "id" | "created_at">) => {
      const { error } = await (supabase.from as any)("order_ratings").insert(
        rating,
      );
      if (error) throw error;
  ...
  const fetchOrderItems = useCallback(
    async (orderId: string): Promise<OrderItemWithProduct[]> => {
      ...
      const { data: items, error } = await supabase
        .from("order_items")
        .select(
          "*, products:product_id(image_url, name, price, markup_enabled)",
        )
        .eq("order_id", orderId)
        .order("created_at");
      ...
      const enrichedItems: OrderItemWithProduct[] = (items || []).map(
        (item: any) => ({
          ...item,
