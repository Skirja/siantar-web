...
export function AdminPanel() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter((order) => {
      ...
      // 2. Village Filter
      if (villageFilter !== "all" && order.customer_village !== villageFilter)
        return false;

      // 3. Outlet Filter
      if (outletFilter !== "all" && order.outlet_id !== outletFilter)
        return false;
  ...
  const [showDeleteOutletConfirm, setShowDeleteOutletConfirm] = useState<
    string | null
  >(null);
  const [showAssignDriverConfirm, setShowAssignDriverConfirm] = useState<{
    orderId: string;
    driverId: string;
  } | null>(null);
  const [showRejectOrderConfirm, setShowRejectOrderConfirm] = useState<
    string | null
  >(null);
  const [showDeleteOrderConfirm, setShowDeleteOrderConfirm] = useState<
    string | null
  >(null);
  const [showRejectPaymentModal, setShowRejectPaymentModal] = useState<
    string | null
  >(null);
  const [rejectPaymentReason, setRejectPaymentReason] = useState("");
  const [showOrderItemsDetail, setShowOrderItemsDetail] = useState<{
    orderId: string;
    outletName: string;
  } | null>(null);
  ...
  const [outletImagePreview, setOutletImagePreview] = useState<string | null>(
    null,
  );
  ...
  const [selectedInvoiceType, setSelectedInvoiceType] = useState<
    "customer" | "outlet"
  >("customer");
  const [showInvoiceTypeSelector, setShowInvoiceTypeSelector] = useState<
    string | null
  >(null);
  ...
  const handleOutletImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onload = (ev) =>
        setOutletImagePreview(ev.target?.result as string);
  ...
  const handleSaveOutlet = async () => {
    ...
    try {
      ...
      const data = {
        ...
        image_url: imageUrl,
        latitude:
          outletForm.latitude !== "" ? Number(outletForm.latitude) : null,
        longitude:
          outletForm.longitude !== "" ? Number(outletForm.longitude) : null,
      };
      if (editingOutlet) {
                          ...
                          <div className="flex justify-between items-start mb-2">
                            <div>
                              ...
                              <div className="text-[10px] text-gray-500">
                                Update:{" "}
                                {(driver as any).last_location_update
                                  ? new Date(
                                      (driver as any).last_location_update,
                                    ).toLocaleTimeString()
                                  : "N/A"}
                          ...
                          <div className="text-xs text-gray-600 mb-3 space-y-1">
                            ...
                            <div className="flex justify-between">
                              ...
                              <span className="font-mono text-[10px]">
                                {(driver as any).latitude?.toFixed(4)},{" "}
                                {(driver as any).longitude?.toFixed(4)}
                              </span>
                            </div>
                          </div>
                    ...
                    {orders.slice(0, 5).map((order) => (
                      ...
                      >
                        ...
                        <div className="text-right">
                          ...
                          <div
                            className={`inline-block px-3 py-1 rounded-full text-xs ${statusColors[order.status]}`}
                          >
            ...
            <div className="space-y-4">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-600 mt-1">
                    Kelola pesanan yang masuk dan sedang berjalan
                  </p>
                </div>
                <button
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 mb-6">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                  ...
                  {/* Stats Summary */}
                  <div
                    className={`flex items-end pb-1 ${dateFilter !== "custom" ? "lg:col-span-2" : ""}`}
                  >
                      ...
                      <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-4">
                        <div className="flex-1">
                          ...
                          {(order.status === "pending" ||
                            ...
                            (() => {
                              const sameVillageOrders = orders.filter(
                                (o) =>
                                  o.id !== order.id &&
                                  o.customer_village ===
                                ...
                                return (
                                  <div className="mb-3 px-3 py-2 bg-orange-50 border border-orange-200 rounded-lg flex items-center gap-2 animate-pulse">
                                    ...
                                    <span className="text-xs font-bold text-orange-700">
                                      {sameVillageOrders.length} pesanan lain di{" "}
                                      {order.customer_village} - Bisa digabung!
                                    </span>
                                  </div>
                                );
                              }
                          ...
                          <div className="space-y-2 text-sm">
                            ...
                            <div className="flex items-start gap-2 text-gray-600">
                              ...
                              <div className="flex-1">
                                ...
                                {order.customer_note && (
                                  <div className="mt-2 flex items-start gap-2 text-xs leading-relaxed text-amber-800 bg-amber-50 p-2.5 rounded-lg border border-amber-300">
                                ...
                                {order.customer_latitude && (
                                  <div className="mt-2 flex items-center gap-2">
                                    <span className="text-[10px] font-mono bg-gray-100 px-2 py-1 rounded border border-gray-200">
                                      {order.customer_latitude.toFixed(6)},{" "}
                                      {order.customer_longitude?.toFixed(6)}
                                    </span>
                                    <a
                            ...
                            <div className="bg-blue-50 border border-blue-200 rounded-lg p-2 mt-2">
                              <div className="text-gray-700">
                                ...
                                {order.distance} km → Dikenakan:{" "}
                                {order.charged_distance} km
                              </div>
                              <div className="text-gray-600 text-xs mt-1">
                                Ongkir: {formatCurrency(order.delivery_fee)}
                              </div>
                            </div>
                            {order.driver_name && (
                              <div className="text-gray-600">
                              ...
                              <div className="space-y-1 text-sm">
                                <div className="text-gray-700">
                                  ...
                                  {order.payment_method === "cod"
                                    ? "COD"
                                    : `Transfer ${order.payment_provider || ""}`}
                                </div>
                                ...
                                {order.payment_status ===
                                  ...
                                  <div className="bg-orange-50 border border-orange-200 rounded-lg p-3 mt-2">
                                    <p className="text-[11px] text-orange-800 font-medium mb-2 italic">
                                      ⚠️ Pesanan Transfer: Konfirmasi
                                      ketersediaan driver & kedai dulu sebelum
                                      customer bayar.
                                    </p>
                                    <div className="flex gap-2">
                                      <button
                                        onClick={async () => {
                                          if (
                                            confirm(
                                              "Konfirmasi pesanan ini? Customer akan melihat instruksi pembayaran.",
                                            )
                                          ) {
                                            try {
                                              ...
                                              toast.success(
                                                "Pesanan dikonfirmasi. Customer sekarang bisa bayar.",
                                              );
                                            } catch (err: any) {
                                              toast.error(err.message);
                                            }
                                      ...
                                      <button
                                        onClick={() =>
                                          setShowRejectOrderConfirm(order.id)
                                        }
                                ...
                                {order.payment_status ===
                                  ...
                                  <div className="flex gap-2 mt-2">
                                    ...
                                    <button
                                      onClick={() =>
                                        setShowRejectPaymentModal(order.id)
                                      }
                                      className="flex-1 px-3 py-2 bg-red-600 text-white rounded text-xs hover:bg-red-700"
                                    >
                                      Tolak
                                    </button>
                                  </div>
                        ...
                        <div className="lg:text-right">
                          ...
                          <div className="bg-orange-50 rounded-lg p-4 space-y-2 text-sm">
                            ...
                            <div className="flex justify-between gap-6">
                              <span className="text-gray-600">
                                Ongkir ({order.charged_distance} km):
                              </span>
                              <span className="font-medium">
                              ...
                              "awaiting_admin_confirmation" ? (
                                ...
                                <div className="bg-white border-2 border-orange-500 rounded-lg p-3">
                                  <div className="text-sm font-medium text-gray-900 mb-2">
                                    Pilih Driver Terdekat:
                                  </div>
                                  <div className="space-y-2">
                                    {(() => {
                                      const orderOutlet = outlets.find(
                                        (o) => o.id === order.outlet_id,
                                      );
                                      const driverList = drivers
                                        ...
                                        .map((driver) => {
                                          ...
                                          if (
                                            isOnline &&
                                            orderOutlet?.latitude &&
                                            orderOutlet?.longitude &&
                                            (driver as any).latitude &&
                                      ...
                                      return driverList.map((driver, idx) => {
                                        ...
                                        const errorReason = !driver.isOnline
                                          ? "Driver offline"
                                          : null;
                                        const warningReason =
                                            ...
                                            >
                                              <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-2">
                                                  <span
                                                    className={`w-2 h-2 rounded-full flex-shrink-0 ${driver.isOnline ? "bg-green-500" : "bg-gray-400"}`}
                                                  />
                                                  <span className="font-medium">
                                                    {driver.name}
                                                    {driver.activeOrdersCount >
                                                      0 && (
                                                      <span className="ml-1 text-[9px] bg-blue-100 text-blue-700 px-1 rounded">
                                                        {
                                                          driver.activeOrdersCount
                                                        }{" "}
                                                        aktif
                                                    ...
                                                    {idx === 0 &&
                                                      driver.isOnline &&
                                                      canAssign && (
                                                        <span className="ml-2 text-[10px] bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full uppercase">
                                                          Terdekat
                                                        </span>
                                                      )}
                                                  </span>
                                                ...
                                                <div className="text-[10px] text-gray-500 flex gap-1 items-center">
                                                  {driver.distToOutlet !==
                                                    Infinity && (
                                                    <span className="font-bold text-orange-600 bg-orange-50 px-1.5 rounded">
                                                      {driver.distToOutlet.toFixed(
                                                        1,
                                                      )}{" "}
                                                      KM ke Kedai
                                                    </span>
                                    ...
                                    <button
                                      ...
                                      className="w-full px-3 py-2 text-gray-600 hover:bg-gray-100 rounded text-sm font-bold border-t mt-2"
                                    >
                                      Batal
                                    </button>
                                  </div>
                            ...
                            {showInvoiceTypeSelector === order.id ? (
                              <div className="bg-white border-2 border-orange-500 rounded-lg p-3">
                                ...
                                <div className="space-y-2">
                                  <button
                                    onClick={() =>
                                      handleSelectInvoiceType("customer")
                                    }
                                  ...
                                  <button
                                    onClick={() =>
                                      handleSelectInvoiceType("outlet")
                                    }
                                    className="w-full text-left px-3 py-2 bg-gray-50 hover:bg-orange-50 rounded text-sm"
                                  >
                                    Invoice Outlet
                          ...
                          {order.status === "pending" && (
                            <div className="mt-3 grid grid-cols-2 gap-2">
                              ...
                              <button
                                onClick={() =>
                                  setShowDeleteOrderConfirm(order.id)
                                }
                ...
                <div className="space-y-6 pb-8 border-b border-gray-100">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        Buka atau tutup seluruh layanan SiAnter
                      </p>
                    </div>
                    <button
                      onClick={() =>
                  ...
                  {!appSettings.is_service_open && (
                    <div className="space-y-4 bg-orange-50 p-4 rounded-lg border border-orange-100">
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">
                          Jam Operasional (Teks)
                        </label>
                        <input
                          type="text"
                          value={appSettings.service_hours || ""}
                          onChange={(e) =>
                            updateAppSetting("service_hours", e.target.value)
                          }
                          className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white"
                      ...
                      <div className="space-y-2">
                        {WILAYAH_1.map((village) => {
                          ...
                          return (
                            ...
                            >
                              <span
                                className={`text-sm font-medium ${isInactive ? "text-gray-400 line-through" : "text-gray-700"}`}
                              >
                                {village}
                              </span>
                              <button
                ...
                <div className="pt-8 border-t border-gray-100">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(appSettings.menu_categories || []).map((cat: string) => (
                      <div
                        key={cat}
                        className="group flex items-center gap-2 px-3 py-1.5 bg-orange-50 text-orange-700 rounded-full border border-orange-100 transition-all hover:bg-orange-100"
                      >
                  ...
                  {outlets.map((outlet) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          isActive &&
                          navigate(`/admin/outlet/${outlet.id}/menu`)
                        }
                        className={`bg-white rounded-xl shadow-sm p-6 transition-shadow ${isActive ? "cursor-pointer hover:shadow-md" : "opacity-75"}`}
                      >
                          ...
                          <div className="flex items-center gap-3">
                            {outlet.image_url ? (
                              <img
                                ...
                                alt={outlet.name}
                                className={`w-12 h-12 rounded-lg object-cover ${!isActive ? "grayscale" : ""}`}
                              />
                            ) : (
                          ...
                          >
                            {isActive ? (
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    if (!outlet.is_manual_schedule) {
                                      toast.info(
                                        "Mode Manual Aktif Otomatis!",
                                        {
                                          description:
                                            "Jadwal otomatis tidak akan berlaku besok kecuali mode ini dimatikan lewat 'Edit'.",
                                        },
                                      );
                                    } else {
                                      toast.success(
                              ...
                              <button
                                onClick={async () => {
                                  if (
                                    confirm(`Pulihkan kedai ${outlet.name}?`)
                                  ) {
                                    try {
                                      await restoreOutlet(outlet.id);
                                    } catch (err: any) {
                                      console.error(
                                        "Gagal memulihkan kedai",
                                        err,
                                      );
                                    }
                                  }
          ...
          <div className="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setOutletForm({ ...outletForm, village: e.target.value })
                  }
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white"
                >
              ...
              <div>
                ...
                >
                  ...
                  <optgroup label="Snack & Jajanan">
                    ...
                    <option value="Cilok, Bakso Bakar & Jajanan">
                      Cilok, Bakso Bakar & Jajanan
                    </option>
                  ...
                  <optgroup label="Minuman">
                    ...
                    <option value="Jus & Minuman Buah">
                      Jus & Minuman Buah
                    </option>
                    <option value="Es Campur / Es Tradisional">
                      Es Campur / Es Tradisional
                    </option>
                  </optgroup>
                  <optgroup label="Lainnya">
                    <option value="Frozen Food">Frozen Food</option>
                    <option value="Catering / Nasi Box">
                      Catering / Nasi Box
                    </option>
                  </optgroup>
                </select>
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <label className="text-sm font-medium text-gray-900">
                      Tambah +Rp1.000 (Semua Menu)
                    </label>
                  </div>
                  <p className="text-[10px] text-gray-600 mt-1">
                    Aktifkan untuk menerapkan markup harga Rp1.000 pada semua
                    menu di kedai ini secara otomatis.
                  </p>
                </div>
                <button
                  onClick={() =>
                    setOutletForm({
                      ...outletForm,
                      markup_enabled: !outletForm.markup_enabled,
                    })
                  }
              ...
              <div className="pt-2">
                ...
                <p className="text-[10px] text-gray-500 mt-1 italic">
                  * Buka Google Maps, tahan pada titik lokasi, lalu copy-paste
                  koordinat jika manual.
                </p>
              </div>
              {/* Jadwal Operasional */}
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() =>
                        setOutletForm({
                          ...outletForm,
                          is_manual_schedule: !outletForm.is_manual_schedule,
                        })
                      }
                    ...
                    >
                      <span
                        className={`inline-block h-3 w-3 transform rounded-full bg-white transition-transform ${outletForm.is_manual_schedule ? "translate-x-5" : "translate-x-1"}`}
                      />
                    </button>
                  </div>
                </div>
                ...
                {outletForm.is_manual_schedule ? (
                  <div className="bg-orange-50 p-3 rounded-lg border border-orange-100 mb-4">
                    <p className="text-xs text-orange-800 mb-2 font-medium">
                      Mode manual aktif. Status outlet ditentukan langsung oleh
                      tombol Buka/Tutup di daftar outlet.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3 mb-4">
                    {Object.entries(outletForm.operating_hours).map(
                    ...
                    <div className="relative">
                      ...
                      <button
                        onClick={() => {
                          setOutletImageFile(null);
                          setOutletImagePreview(null);
                          setOutletForm({ ...outletForm, image_url: null });
                        }}
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button
                ...
                className="flex-1 px-4 py-3 bg-orange-500 text-white rounded-lg hover:bg-orange-600 font-medium disabled:opacity-50"
              >
                {savingOutlet ? (
              ...
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <div className="flex items-start gap-2">
                  ...
                  <div className="text-sm text-yellow-800">
                    ...
                    <div>
                      Customer akan melihat alasan ini agar mereka memahami
                      mengapa bukti pembayaran ditolak.
                    </div>
                  </div>
                </div>
              </div>
